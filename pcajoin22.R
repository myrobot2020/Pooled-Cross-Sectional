...
fei<-function(x) {x %>% group_by(village_town_mdds) %>% summarise(across(contains("population"),sum))}


df_list<-df_list|>map(fee)|>map(fei) 


colnames(df_list$`1951.xlsx`)[-1]<-paste(names(df_list$`1951.xlsx`)[-1],"1951",sep = "-")
colnames(df_list$`1961.xlsx`)[-1]<-paste(names(df_list$`1961.xlsx`)[-1],"1961",sep = "-")

df_list1<-reduce(df_list,full_join,by='village_town_mdds')


###############################################
mrMDDS <- read_excel("mrMDDS.xls")
df_list<-merge(mrMDDS,df_list,"village_town_mdds")
########################################
